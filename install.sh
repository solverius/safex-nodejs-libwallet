...
then
	...
	cp /usr/lib/gcc/x86_64-w64-mingw32/*-posix/libgcc_s_seh-1.dll lib/binding/
	cp /usr/lib/gcc/x86_64-w64-mingw32/*-posix/libstdc++-6.dll lib/binding/
else
	mkdir -p mkdir -p $(pwd)/build;
	make deps && cmake-js build --CDARCH='x86-64' --CDBUILD_WIN=OFF --CDBUILD_64=ON --arch=x86_64 -m;
	mkdir -p mkdir -p $(pwd)/buildlib/binding;
